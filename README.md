...
It tests the packet parser, the CSV writer, the port detection and the whole
window, without needing a radio or a CanSat. You should see a list of `ok`
lines ending in:

```
All 40 checks passed. The installation is good.
```
...
| It says | Meaning | Fix |
|---|---|---|
| `No XBee found — check the USB cable` | No USB serial device at all | Check the cable; install the adapter driver (step 4) |
...
| File | Contains |
|---|---|
| `ground_station_simple.py` | Entry point; just calls `gs_ui.main()` |
| `gs_logic.py` | Everything that is not drawing — serial link, packet parser, mission data, CSV writer, commands, simulation |
| `gs_ui.py` | All the Qt code — widgets, tabs, main window |

`gs_logic.py` imports no Qt, so it can be tested on its own.
